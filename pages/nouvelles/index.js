...
export default function Blog({ allbloglist }) {
  ...
  return (
    <>
      ...
      <BreadHero
        heading="Nouvelles"
        currentPage="Nouvelles"
      />


      <SpaceMy>
        <div className='text-center'>
          <PageHeading title={'Toutes les Nouvelles'} />
        </div> 
        <SpaceMy bottomSpczero={true}>
          <BlogTile allbloglist={allbloglist} path={`nouvelles`} readmore={`Read more`} noitem={`No items found !`} admintext={`Voyage`} />
        </SpaceMy>
      </SpaceMy> 
    </>
  )
